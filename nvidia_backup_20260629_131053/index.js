...
async function probeModel(modelId) {
  ...
  try {
    ...
    } else if (resp.status === 200 || resp.status === 400 || resp.status === 422) {
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      const resp = await undiciFetch(url, {
        ...
        body: JSON.stringify(body),
        dispatcher: agent,
        signal: AbortSignal.timeout(parseInt(process.env.REQUEST_TIMEOUT || '600', 10) * 1000),
      });
      ...
      if (!resp.ok && resp.status !== 200) {
        ...
        pool.releaseSuccess(key);
        return { status: resp.status, data: errBody || { error: { message: `Upstream ${resp.status}`, type: 'upstream_error' } } };
      }
...
async function handleModels(res, url = null) {
  ...
  const data = ids.map(id => ({ id, object: 'model', owned_by: 'nvidia', created: 0 }));
...
async function handleCatchAll(req, res, path, url) {
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      const contentType = resp.headers.get('content-type') || '';
      if (isStreaming || contentType.includes('text/event-stream') || resp.body) {
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    if (method === 'GET' && (path === '/version' || path === '/api/version')) {
      return jsonResp(res, 200, { version: "wrapper-nvidia-4.1.0" });
    }
