...
class Metrics {
  ...
  _save(sync = false) {
    ...
    // FIX M4 (audit-2026-06-30): Concurrency-safe save. Multiple intervals/handlers
    // could previously fire _save() concurrently and either write a stale buffer or
    // hit ENOENT on rename when one path deleted .tmp while the other was still writing.
    if (this._db === null) return;
    if (this._saveInFlight) {
      // Coalesce: a save is already in progress; skip this one.
      return;
    }
    ...
    try {
      ...
      const finalize = (writeErr) => {
        let renameOk = true;
        const doRename = (cb) => {
          fs.rename(tmpPath, this._dbPath, (renameErr) => {
            // ENOENT _on the tmp file_ during rename means tmp wasn't created.
            // (Happens on rapid boot/restart or if dir was wiped mid-write.)
            // Silent skip is OK if there's no tmp to rename — main db is still intact.
            if (renameErr) {
              if (renameErr.code === 'ENOENT') {
                console.warn('[metrics] _save() rename ENOENT (tmp gone) — main DB intact');
              } else {
        ...
        if (writeErr) {
          console.error('[metrics] _save() write failed:', writeErr.message);
          this._saveInFlight = false;
        } else if (sync) {
          try {
            ...
            if (renameErr.code !== 'ENOENT') {
              console.error('[metrics] _save() sync rename failed:', renameErr.message);
            }
          }
          this._saveInFlight = false;
        } else {
      ...
      if (sync) {
        try {
          // Ensure parent directory exists (mtime protection for ENOENT on tmp dir).
          const dir = require('path').dirname(this._dbPath);
          if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
          fs.writeFileSync(tmpPath, buffer);
          finalize(null);
        } catch (e) {
          console.error('[metrics] _save() sync write failed:', e.message);
          this._saveInFlight = false;
        }
      } else {
        // Ensure parent dir exists for async path too.
        const dir = require('path').dirname(this._dbPath);
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        fs.writeFile(tmpPath, buffer, finalize);
      }
    } catch (e) {
