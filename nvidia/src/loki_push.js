...
function pushChunk() {
  ...
  const streams = snapshot.map((line, idx) => ({
    stream: LABELS,
  ...
  const req = mod.request(url, {
    ...
    if (res.statusCode >= 200 && res.statusCode < 300) {
      // Only clear batch on successful push
      batch = batch.slice(snapshot.length);
      console.log(`[loki_push] flushed ${snapshot.length} records`);
    } else {
