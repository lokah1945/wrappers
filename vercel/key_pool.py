#!/usr/bin/env python3
"""
key_pool.py — resilient multi-key pool for Vercel Zen.
...
class KeyEntry:
    ...
    def block(self, seconds: int, reason: str):
        ...
        if reason == 'rate_limit':
            self.total_429s += 1
        logger.warning(f'[vercel] key {self.label} cooled down for {seconds}s ({reason})')
...
class KeyPool:
    ...
    def load_from_env(self):
        ...
        for key_name, value in sorted(os.environ.items()):
            if not value or len(value.strip()) < 10:
                continue
            if key_name == 'VERCEL_API_KEY' or key_name.startswith('VERCEL_API_KEY_'):
                v = value.strip()
                if v in seen:
                    continue
                seen.add(v)
        ...
        if env_keys:
            ...
            logger.warning('[vercel] No VERCEL_API_KEY* found')

        logger.info(f'[vercel] Loaded {len(self.keys)} Vercel key(s) soft={self.soft_limit} hard={self.hard_limit}')
        return self
    ...
    async def acquire(self, model: str = '') -> Optional[dict]:
        ...
        async with self._lock:
            ...
            if sum(k.in_flight for k in self.keys) >= inflight_cap:
                logger.warning(f'[vercel] Load shedding: in-flight >= {inflight_cap}')
                return None
    ...
    def prom_metrics(self) -> str:
        lines = [
            '# HELP vercel_keys_total Total keys',
            '# TYPE vercel_keys_total gauge',
            f'vercel_keys_total {self.total_keys}',
            '# HELP vercel_keys_available Available keys',
            '# TYPE vercel_keys_available gauge',
            f'vercel_keys_available {self.available_keys}',
            '# HELP vercel_in_flight_total In flight',
            '# TYPE vercel_in_flight_total gauge',
            f'vercel_in_flight_total {self._in_flight_total}',
        ]
        for k in self.keys:
            st = k.stats(self.soft_limit, self.hard_limit)
            lines.append(f'vercel_key_rpm{{key="{k.label}"}} {st["current_rpm"]}')
            lines.append(f'vercel_key_blocked{{key="{k.label}"}} {1 if st["hard_blocked"] else 0}')
            lines.append(f'vercel_key_failures_total{{key="{k.label}"}} {st["total_failures"]}')
        return '\n'.join(lines) + '\n'
